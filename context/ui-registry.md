...
**Status:** implemented
**Feature ID:** RF-ADM-013 / RF-ADM-022 / RF-DOC-003 / RF-DOC-004
...
**Purpose:** Dense admin accountant-export preparation surface for approved shift rows, monthly payroll totals, export filters and audit-logged CSV/XLSX download workflows.
...
- admin-only XLSX download action with loading, disabled and success/error states
...
- CSV and XLSX export generation uses approved shifts only, `billable_minutes`, company scope and `accountant_export_created` audit logging.
- XLSX export uses the same server-side export data definition as CSV and adds a single selected-month sheet with a totals row.
...
- Backend-backed export pages can use matched active format actions; RF-DOC-004 uses `CSV herunterladen` / `CSV wird erstellt` and `XLSX herunterladen` / `XLSX wird erstellt`.
...
**Status:** implemented
**Feature ID:** RF-BE-013 / RF-DOC-001 / RF-DOC-002
**Path:** `apps/mobile/app/(tabs)/history.tsx`, `apps/mobile/app/history/[date].tsx`, `apps/mobile/features/history/historyHydration.ts`, `apps/mobile/features/history/dailyPdfDownload.ts`
...
- Daily and monthly PDF actions use the existing large rounded primary action pattern and authenticated admin API downloads instead of public storage links.
- Daily PDF actions are enabled only for backend-loaded shift details; monthly PDF actions are enabled only after backend month history has loaded without an error.
- PDF loading/success/error feedback stays as compact text below the action button, using `text-rfPrimary` for success and `text-rfWarningForeground` for errors.
